...
logging.basicConfig(
    level=logging.DEBUG,  # Using DEBUG for more detailed logs
...
def main():
    ...
    if all_events:
        try:
            ...
            output_file = './data/index_space_events.json'
            ...
            try:
                output_file = 'index_space_events.json'
                with open(output_file, 'w') as f:
                    json.dump({"events": all_events}, f, indent=2)
                logging.info(f"Saved {len(all_events)} events to {output_file}")
            except Exception as e2:
                logging.error(f"Failed to save events to alternative path: {str(e2)}")
    else:
