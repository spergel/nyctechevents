...
def generate_tweet_with_gemini(event):
    ...
    try:
        ...
        Only generate a tweet if the event title and description sound like a public event of genuine interest; if it sounds like a placeholder, private booking, or test, respond with only the text "SKIP_EVENT".
        Max 280 characters. Directly provide only the tweet text or "SKIP_EVENT"."""
        ...
        if generated_tweet == "SKIP_EVENT":
            logging.info(f"Gemini suggested skipping event: {event_name}")
            return None # Return None to indicate skipping this event
...
def main():
    ...
    try:
        ...
        for i, event in enumerate(upcoming_events):
            ...
            if not tweet_content: # If Gemini returned None (e.g. "SKIP_EVENT")
                logging.info(f"Skipping tweet post for event {event.get('name', 'Unnamed Event')} as no content was generated.")
